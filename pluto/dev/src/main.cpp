...
int main(int argc, char *argv[])
{
    ...
    for (int k = 0; k < 15000000; k++)
    {
        ...
        sr = sdr1.sdr->readStream(sdr1.rxStream, rx_buffs, sdr1.buffer_size, flags, timeNs, timeoutUs);
        long long tx_time = timeNs + (4 * 1000 * 1000); // Schedule TX 4ms ahead
        if (strcmp(argv[2], "rx") != 0)
        {
            st = sdr1.sdr->writeStream(sdr1.txStream, (const void *const *)tx_buffs, sdr1.buffer_size, flags, tx_time, timeoutUs);
            (void)st;
        }
